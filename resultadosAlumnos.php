...
<h1 class="mt-5">Buscador de Alumnos Online</h1>
...
<?php
  ...
  $buscar = $con->query("select * from alumno where ".$tipobusqueda." like '%".$terminobusqueda."%'");
  ...
  for ($i=0; $i <$num_resultados; $i++)
  {
   $row = mysqli_fetch_array($buscar);
     echo "<p><strong>".($i+1).". Nombre: </strong>";
     echo stripslashes($row["nombre"]."&nbsp;". $row["APaterno"] ."&nbsp;".$row["AMaterno"]);
     echo "</p>";
  }
